...
class GenerateXLSXReport(models.Model):
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        if data['location_ids']:            
            ...
            all_location_ids = self.env['stock.location'].search(['|',('id', 'in', data['location_ids']),('id', 'child_of', data['location_ids'])])
        ...
        for product in products_list:
            ...
            total_quant_ids = self.env['stock.quant'].search([('product_id','=',product.id),('location_id.usage','=','internal')])
            ...
            move_lines = self.env['stock.move.line'].search([('date','<=',data['in_date']),('state','=','done'),'|',('location_id', 'in', data['location_ids']),('location_id', 'child_of', data['location_ids'])])
            move_dest_lines = self.env['stock.move.line'].search([('date','<=',data['in_date']),('state','=','done'),'|',('location_dest_id', 'in', data['location_ids']),('location_dest_id', 'child_of', data['location_ids'])])
            ...
            stock_valuation_id = self.env['stock.valuation.layer'].search([('product_id','=',product.id),('unit_cost','!=',0),('create_date','<=',data['in_date'])],order='unit_cost',limit=1)
            min_price = stock_valuation_id.unit_cost
            stock_valuation_id = self.env['stock.valuation.layer'].search([('product_id','=',product.id),('unit_cost','!=',0),('create_date','<=',data['in_date'])],order='unit_cost desc',limit=1)
