...
class CustomerPortal(CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if 'penalty_count' in counters:
            values['penalty_count'] = request.env['account.custom.entry'].sudo().search_count([('custom_entry_type_id.penalty_published','=',True),('partner_id', '=', active_user.partner_id.id)])
        return values
  
    def _penalty_get_page_view_values(self,penalty, next_id = 0,pre_id= 0, entry_user_flag = 0, access_token = None, **kwargs):
        values = {
            'page_name' : 'penalty',
            'penalty' : penalty,
        ...
        return self._get_page_view_values(penalty, access_token, values, 'my_entry_history', False, **kwargs)

    @http.route(['/penalty/orders', '/penalty/orders/page/<int:page>'], type='http', auth="user", website=True)
    def portal_my_penalty_entries(self, page=1, date_begin=None, date_end=None, sortby=None, filterby=None, search=None,
        ...
        project_groups = request.env['account.custom.entry'].sudo().search([('custom_entry_type_id.penalty_published','=',True),('partner_id','=', active_user.partner_id.id)])
...
#         domain = []
        ...
        domain += [('custom_entry_type_id.penalty_published','=',True)]
        ...
        penalty_count = request.env['account.custom.entry'].sudo().search_count(domain)
        ...
        _entrys = request.env['account.custom.entry'].sudo().search(domain, order=order, limit=1000, offset=pager['offset'])
        request.session['my_entry_history'] = _entrys.ids[:100]

        grouped_penalty = [_entrys]
                
        paging(0,0,1)
        paging(grouped_penalty)
        
        values.update({
            'date': date_begin,
            'date_end': date_end,
            'grouped_penalty': grouped_penalty,
            'page_name': 'penalty',
            'default_url': '/penalty/orders',
            'pager': pager,
        ...
        return request.render("de_portal_penalty_journal_entry.portal_my_entry_penalty", values)

   
    @http.route(['/penalty/order/<int:move_id>'], type='http', auth="user", website=True)
    def portal_my_penalty_entry(self, move_id, access_token=None, **kw):
        values = []
        ...
        values = self._penalty_get_page_view_values(entry_sudo,next_id, pre_id, entry_user_flag,access_token, **kw)
        return request.render("de_portal_penalty_journal_entry.portal_custom_entry_penalty", values)

    @http.route(['/penalty/order/next/<int:move_id>'], type='http', auth="user", website=True)
    def portal_my_next_penalty_entry(self, move_id, access_token=None, **kw):
        ...
        values = self._penalty_get_page_view_values(entry_sudo,next_id, pre_id, access_token, **kw)
        return request.render("de_portal_penalty_journal_entry.portal_custom_entry_penalty", values)

  
    @http.route(['/penalty/order/pre/<int:move_id>'], type='http', auth="user", website=True)
    def portal_my_pre_penalty_entry(self, move_id, access_token=None, **kw):
