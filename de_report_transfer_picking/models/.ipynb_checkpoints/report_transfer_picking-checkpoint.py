...
class GenerateXLSXReport(models.Model):
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        for order in transfer_order_ids.filtered(lambda r: r.transfer_order_type_id.code == 'SPF'):
            ...
            for picking_id in order.picking_ids.filtered(lambda x: x.picking_type_id.id == order.picking_type_id.id):
                ...
                if order.delivery_deadline:
                    sheet.write(row, 12, order.delivery_deadline.strftime("%d/%m/%Y"), format2)
                if order.return_deadline:
                    sheet.write(row, 13, order.return_deadline.strftime("%d/%m/%Y"), format2)
                ...
                for move in picking_id.move_lines:
                    ...
                    stock_valuation_id = self.env['stock.valuation.layer'].search([('stock_move_id','=', move.id)],limit=1)
                    product_value = stock_valuation_id.value
                    sheet.write(row, 3, product_value, format2)
                
                    request_lines = self.env['stock.transfer.order.line'].search([('stock_transfer_order_id','=',order.id),('product_id','=',move.product_id.id)])
                    request_qty = delivered_qty = 0
                    for line in request_lines:
                        request_qty += line.product_uom_qty
                        delivered_qty += line.delivered_qty
                        
                    return_lines = self.env['stock.transfer.return.line'].search([('stock_transfer_order_id','=',order.id),('product_id','=',move.product_id.id)])
                    return_qty = received_qty = 0
        ...
        for order in transfer_order_ids.filtered(lambda r: r.transfer_order_type_id.code == 'MRF'):
            picking_id = self.env['stock.picking'].search([('stock_transfer_order_id','=',order.id),('picking_type_id','=',order.picking_type_id.id)],limit=1)
        ...
        for order in transfer_order_ids.filtered(lambda r: r.transfer_order_type_id.code == 'SPF'):
            ...
            for picking_id in order.picking_ids.filtered(lambda x: x.picking_type_id.id == order.picking_type_id.return_picking_type_id.id):
                sheet.set_column(row, 0, 50)
                sheet.set_column(row, 1, 25)
                sheet.set_column(row, 2, 20)
                sheet.set_column(row, 3, 20)
